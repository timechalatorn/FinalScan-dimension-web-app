...
async def upload_image(
    ...
    for i, box in enumerate(everything_results[0].boxes):
        ...
        if len(contours) > 0:
            ...
            cv2.putText(frame, f'ID: {i}', (int(box[0]), int(box[1]) - 50), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 3)
...
async def process_image(
    ...
    for i, box in enumerate(everything_results[0].boxes):
        ...
        if len(contours) > 0:
            ...
            cv2.putText(frame, f'W: {height_real_world:.2f} cm', (int(box[0]), int(box[1]) - 210), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 3)
            cv2.putText(frame, f'H: {width_real_world:.2f} cm', (int(box[0]), int(box[1]) - 130), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 3)
